      ...
      <header className="sticky top-0 z-50 w-full border-b border-white/10 bg-background/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="font-bold text-lg tracking-tight bg-gradient-to-r from-white via-white to-violet-300 bg-clip-text text-transparent">
              Factora
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-3">
            {isConnected && walletName === 'Simulated' && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-violet-500/30 bg-violet-500/10 text-violet-300 text-xs font-semibold hover:bg-violet-500/20 disabled:opacity-50 transition-all cursor-pointer"
              >
            ...
            {isConnected && (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 text-xs font-semibold transition-colors cursor-pointer"
                >
                  ...
                  {showRoleDropdown && (
                    <>
                      ...
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: 10 }}
                        className="absolute right-0 mt-1.5 w-40 rounded-lg border border-white/10 bg-card p-1 shadow-xl z-20"
                      >
                        {(['INVESTOR', 'BUSINESS', 'ADMIN'] as UserRole[]).map((r) => (
                          <button
                            ...
                            className="flex items-center justify-between w-full px-3 py-2 text-left text-xs rounded-md hover:bg-white/5 transition-colors cursor-pointer"
                          >
                            <span>{r}</span>
                            {role === r && <Check className="w-3.5 h-3.5 text-violet-500" />}
                          </button>
                        ))}
                      </motion.div>
                    </>
            ...
            {isConnected ? (
              <div className="flex items-center gap-2 pl-2 border-l border-white/10">
                ...
                <button
                  onClick={disconnect}
                  className="p-2 rounded-lg hover:bg-white/5 border border-transparent hover:border-white/10 text-muted-foreground hover:text-destructive transition-all cursor-pointer"
          ...
          <div className="flex md:hidden items-center gap-3">
            {isConnected && (
              <span className="text-xs font-semibold text-violet-300 bg-violet-500/10 border border-violet-500/20 px-2 py-1 rounded">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="md:hidden border-b border-white/10 bg-background/95 backdrop-blur-lg w-full overflow-hidden"
          >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        {walletName === 'Simulated' && (
                          <button
                            onClick={handleFaucet}
                            className="px-2.5 py-1 rounded bg-violet-500/10 text-violet-300 text-xs border border-violet-500/20"
                          >
                            Faucet
                          </button>
                        )}
        ...
        {showWalletModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-sm rounded-xl border border-white/10 bg-card p-6 shadow-2xl z-10"
            >
              ...
              <div className="grid gap-3">
                <button
                  ...
                  className="flex items-center gap-3 w-full p-3 rounded-lg border border-white/5 bg-white/5 hover:bg-white/10 text-left transition-all cursor-pointer hover:border-violet-500/45 group"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-3 w-full p-3 rounded-lg border border-violet-500/20 bg-violet-500/5 hover:bg-violet-500/10 text-left transition-all cursor-pointer hover:border-violet-500/60 group"
                >
                  <div className="w-8 h-8 rounded bg-violet-500/20 text-violet-400 flex items-center justify-center group-hover:scale-105 transition-transform">
                    <Coins className="w-4 h-4" />
                  </div>
                  <div>
                    <div className="text-sm font-semibold text-violet-300">Simulated Wallet (Demo Mode)</div>
                    <div className="text-xs text-violet-400/70">Instantly test the full platform lifecycle</div>
                  </div>
                </button>
              </div>
            </motion.div>
