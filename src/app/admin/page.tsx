        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xl font-bold text-foreground tracking-tight">Marketplace Invoices Audit</h3>
            </div>

            {invoicesLoading ? (
              <div className="h-32 bg-muted border border-border rounded-xl animate-pulse" />
            ) : !invoices || invoices.length === 0 ? (
              <div className="glass-panel rounded-xl p-8 text-center text-xs text-muted-foreground border border-border">
                    ...
                    {invoices.map((invoice) => {
                      ...
                      return (
                        <tr key={invoice.id} className="hover:bg-muted">
                          ...
                          <td className="px-4 py-4">
                            <span className="px-2 py-0.5 rounded-full text-xxs font-bold border bg-muted border-border">
                              {invoice.status}
                            </span>
                          </td>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xl font-bold text-foreground tracking-tight">Active Platform Users</h3>
            </div>

            {usersLoading ? (
              <div className="h-48 bg-muted border border-border rounded-xl animate-pulse" />
            ) : !users || users.length === 0 ? (
              <div className="glass-panel rounded-xl p-6 text-center text-xs text-muted-foreground border border-border">
                No users found.
              </div>
            ) : (
              <div className="glass-panel p-4 rounded-xl border border-border space-y-4">
                <div className="divide-y divide-white/5 text-xs text-foreground">
                  {users.map((usr) => (
                    <div key={usr.id} className="py-3 first:pt-0 last:pb-0 flex items-center justify-between">
                      ...
                      <span className="px-2 py-0.5 rounded text-xxs font-semibold bg-muted border border-border">
                        {usr.role}
                      </span>
                    </div>
                  ))}
